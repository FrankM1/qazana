...
module.exports = BaseRegion.extend( {
		...
		return {
			...
			stop: () => {
				...
				if ( this.isDocked ) {
					...
					this.storage.size.height = ( qazana.helpers.getElementInlineStyle( this.$el, [ 'height' ] ).height - 200 );
					this.saveSize();
				}
			},
	...
	dock: function() {
		...
		const side = Boolean( qazana.config.is_rtl ) ? 'right' : 'left',
	...
	undock: function( silent ) {
		...
		const side = Boolean( qazana.config.is_rtl ) ? 'left' : 'right';
