...
LightboxModule = ViewModule.extend( {
	...
	getDefaultElements: function() {
		return {
			$lightBoxContainer: jQuery( qazanaFrontend.getScopeWindow().document.body )
		};
	},
	...
	initModal: function() {
		...
		var modal = LightboxModule.modal = qazanaFrontend.getDialogsManager().createWidget( 'lightbox', {
			className: 'qazana-lightbox-modal',
			container: self.elements.$lightBoxContainer,
			closeButton: true,
			position: {
				within: qazanaFrontend.getScopeWindow()
			}
		} );
	...
	setVideoContent: function( videoEmbedURL ) {
		...
		modal.getElements( 'message' ).addClass( 'qazana-video-wrapper' );
