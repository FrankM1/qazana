...
use Qazana\Base_Data_Control;
use Qazana\Controls_Manager;
use Qazana\Controls_Stack;
use Qazana\Core\Files\Base as Base_File;
 ...
 * Qazana CSS file.
 *
 * Qazana CSS file handler class is responsible for generating CSS files.
 *
 * @since 1.2.0
 * @abstract
 */
abstract class Base extends Base_File {
	...
	/**
	 * Qazana CSS file generated status.
	 *
	...
	protected function use_external_file() {
		return 'internal' !== get_option( 'qazana_css_print_method' );
	}
	 ...
	 * Enqueue CSS.
	 *
	 * Either enqueue the CSS file in Qazana or add inline style.
	 *
	...
	public function enqueue() {
		 ...
		 * @param Base $this The current CSS file.
		 */
		do_action( "qazana/css-file/{$name}/enqueue", $this );
	}
	...
	protected function parse_content() {
		 ...
		 * Parse CSS file.
		 *
		 * Fires when CSS file is parsed on Qazana.
		 *
		 * The dynamic portion of the hook name, `$name`, refers to the CSS file name.
		 *
		 * @since 1.2.0
		 * @deprecated 2.0.0 Use `qazana/css-file/{$name}/parse` action instead.
		 * @todo Need to be hard deprecated using `do_action_deprecated()`.
		 *
		 * @param Base $this The current CSS file.
		 */
		do_action( "qazana/{$name}-css-file/parse", $this );
		 ...
		 * @param Base $this The current CSS file.
		 */
		do_action( "qazana/css-file/{$name}/parse", $this );
	...
	protected function add_repeater_control_style_rules( Controls_Stack $controls_stack, array $repeater_controls_items, array $repeater_values, array $placeholders, array $replacements ) {
		...
		foreach ( $repeater_controls_items as $index => $item ) {
			$this->add_controls_stack_style_rules(
				...
				$placeholders,
				array_merge( $replacements, [ '.qazana-repeater-item-' . $repeater_values[ $index ]['_id'] ] )
			);
		}
	}
	...
	protected function add_dynamic_control_style_rules( array $control, $value ) {
		qazana()->get_dynamic_tags()->parse_tags_text( $value, $control, function( $id, $name, $settings ) {
			...
			$this->add_controls_stack_style_rules( $tag, $tag->get_style_controls(), $tag->get_active_settings(), [ '{{WRAPPER}}' ], [ '#qazana-tag-' . $id ] );
		} );
	}
}
