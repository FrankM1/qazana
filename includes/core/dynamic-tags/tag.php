 ...
 * Qazana tag.
 *
 * An abstract class to register new Qazana tag.
 *
 * @since 2.0.0
 * @abstract
 */
abstract class Tag extends Base_Tag {
	...
	public function get_content( array $options = [] ) {
		...
		if ( $value ) {
			// TODO: fix spaces in `before`/`after` if WRAPPED_TAG ( conflicted with .qazana-tag { display: inline-flex; } );
			...
			if ( self::WRAPPED_TAG ) :
				$value = '<span id="qazana-tag-' . esc_attr( $this->get_id() ) . '" class="qazana-tag">' . $value . '</span>';
			endif;
