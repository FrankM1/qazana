...
/**
 * Qazana page settings model.
 *
 * Qazana page settings model handler class is responsible for registering
 * and managing Qazana page settings models.
 *
 * @since 1.6.0
 */
class Model extends Base {
	 ...
	 * Model constructor.
	 *
	 * Initializing Qazana page settings model.
	 *
	 * @since 1.6.0
	 * @access public
	 *
	...
	public function get_panel_page_settings() {
		...
		return [
			...
			'title' => sprintf( __( '%s Settings', 'qazana' ), $document::get_title() ),
		];
	}
	 ...
	 * exclude it from the exported Qazana data.
	 *
	 * @since 1.6.0
	 * @access public
	 *
	...
	public function on_export( $element_data ) {
		if ( ! empty( $element_data['settings']['template'] ) ) {
			/**
			 * @var \Qazana\Modules\PageTemplates\Module $page_templates_module
			 */
			$page_templates_extension = qazana()->get_extensions_manager()->get_extension( 'page_templates' );
			$is_qazana_template = ! !$page_templates_extension->get_template_path( $element_data['settings']['template'] );

			if ( ! $is_qazana_template ) {
