<?php
namespace Qazana\Extensions\Assets_Manager\AssetTypes\Icons;

use Qazana\Extensions\Assets_Manager\Classes;
use Qazana\Extensions\Assets_Manager\AssetTypes\Icons_Manager;

if ( ! defined( 'ABSPATH' ) ) {
	exit; // Exit if accessed directly.
}

class Custom_Icons extends Classes\Icon_Base {

	const FONT_META_KEY = 'qazana_font_files';
	const FONT_FACE_META_KEY = 'qazana_font_face';
	...
	public function add_meta_box() {
		add_meta_box(
			'qazana-font-' . $this->get_type() . 'metabox',
			__( 'Manage Your Icon Files', 'qazana' ),
			[ $this, 'render_metabox' ],
			Icons_Manager::CPT,
			'normal',
			'default'
	...
	public function render_metabox( $post ) {
		...
		$fields = [
			...
			[
				...
				'field_type' => 'html',
				'label' => false,
				'raw_html' => sprintf( '<div class="inline-preview">%s</div>', __( 'Qazana Is Making the Web Beautiful!!!', 'qazana' ) ),
			],
			[
				'id' => 'toolbar',
				'field_type' => 'toolbar',
		...
		foreach ( $this->get_file_types() as $type => $mine ) {
			$fields[] = [
				...
				'ext' => $type,
				/* translators: %s: Icon file format. */
				'label' => sprintf( __( '%s File', 'qazana' ), strtoupper( $type ) ),
				/* translators: %s: Icon file format. */
				'box_title' => sprintf( __( 'Upload font .%s file', 'qazana' ), $type ),
				/* translators: %s: Icon file format. */
				'box_action' => sprintf( __( 'Select .%s file', 'qazana' ), $type ),
				'preview_anchor' => 'none',
		...
		$repeater = [
			...
			'id' => 'font_face',
			'label' => false,
			'add_label' => __( 'Add Icon Variation', 'qazana' ),
			'toggle_title' => __( 'Edit', 'qazana' ),
			'remove_title' => __( 'Delete', 'qazana' ),
			'field_type' => 'repeater',
			'row_label' => [
				'default' => 'Settings',
	...
	private function get_font_face_by_font_family( $font_family ) {
		...
		$id = $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM $wpdb->posts WHERE post_title = %s AND post_type = %s LIMIT 1", $font_family, Icons_Manager::CPT ) );
	 ...
	 * @param \Qazana\CSS_File $post_css
	 */
	public function enqueue_font( $font_family, $font_data, $post_css ) {
		...
		// Add a css comment
		$custom_css = '/* Start Custom Icons CSS */' . $font_faces . '/* End Custom Icons CSS */';
	...
	public function handle_panel_request() {
		...
		if ( empty( $font_face ) ) {
			/* translators: %s: Icon family. */
			$error_message = sprintf( __( 'Icon %s was not found.', 'qazana' ), $font_family );
	...
	private function get_font_style_options() {
		return [
			'normal' => __( 'Normal', 'qazana' ),
			'italic' => __( 'Italic', 'qazana' ),
			'oblique' => __( 'Oblique', 'qazana' ),
		];
	}
	...
	private function get_file_type_description( $file_type ) {
		$descriptions = [
			'eot' => __( 'Embedded OpenType, Used by IE6-IE9 Browsers', 'qazana' ),
			'woff2' => __( 'The Web Open Icon Format 2, Used by Super Modern Browsers', 'qazana' ),
			'woff' => __( 'The Web Open Icon Format, Used by Modern Browsers', 'qazana' ),
			'ttf' => __( 'TrueType Icons, Used for better supporting Safari, Android, iOS', 'qazana' ),
			'svg' => __( 'SVG fonts allow SVG to be used as glyphs when displaying text, Used by Legacy iOS', 'qazana' ),
		];
