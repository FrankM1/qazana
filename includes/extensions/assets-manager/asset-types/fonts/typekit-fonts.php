...
namespace Qazana\Extensions\Assets_Manager\AssetTypes\Fonts;

use Qazana\Extensions\Assets_Manager\Classes\Font_Base;
...
class Typekit_Fonts extends Font_Base {
	...
	const TYPEKIT_FONTS_OPTION_NAME = 'qazana_typekit-data';
	...
	private function get_typekit_kit_id() {
		return get_option( 'qazana_' . self::TYPEKIT_KIT_ID_OPTION_NAME, false );
	}

	public function get_name() {
		return __( 'TypeKit', 'qazana' );
	}
	...
	private function fetch_typekit_data() {
		...
		if ( 200 !== $response_code ) {
			switch ( $response_code ) {
				case 404:
					$this->error = __( 'Kit not found.', 'qazana' );
					break;
				default:
		...
		if ( ! $response_body ) {
			$this->error = __( 'No kit data was returned.', 'qazana' );
	...
	public function handle_panel_request() {
		...
		if ( ! $typekit_fonts || ! is_array( $typekit_fonts ) ) {
			$error_message = __( 'Error with typekit fonts', 'qazana' );
			throw new \Exception( $error_message );
		}
		if ( ! in_array( $font_family, array_keys( $typekit_fonts ) ) ) {
			$error_message = __( 'Font missing in Kit', 'qazana' );
			throw new \Exception( $error_message );
		}
		$kit_id = $this->get_typekit_kit_id();
	...
	public function register_admin_fields( Panel $settings ) {
		...
		$button_label = __( 'Get Kit', 'qazana' );
		$found_label = '<span class="qazana-typekit-count">{{count}}</span> ' . __( 'Fonts Families Found in kit. Please note that typekit takes a few minutes to sync once you publish or update a kit.', 'qazana' );
		if ( $fonts && is_array( $fonts ) ) {
			$button_label = __( 'Sync Kit', 'qazana' );
		}
		$settings->add_section( Panel::TAB_INTEGRATIONS, 'typekit', [
			'callback' => function () {
				echo '<hr><h2>' . esc_html__( 'TypeKit', 'qazana' ) . '</h2>';
				esc_html_e( 'Typekit partners with the world’s leading type foundries to bring thousands of beautiful fonts to designers every day.', 'qazana' );
			},
			'fields' => [
				self::TYPEKIT_KIT_ID_OPTION_NAME => [
					'label' => __( 'Kit ID', 'qazana' ),
					'field_args' => [
						'type' => 'text',
						'desc' => sprintf( __( 'Enter Your Typekit Kit ID', 'qazana' ) ),
					],
					'setting_args' => [
				...
				'validate_api_data' => [
					'field_args' => [
						'type' => 'raw_html',
						'html' => sprintf( '<button data-found="%s" data-action="%s" data-nonce="%s" class="button qazana-button-spinner" id="qazana_typekit_validate_button">%s</button><br><p><span class="qazana-typekit-data hidden"></span></p>',
	...
	public function integrations_admin_ajax_handler() {
		...
		$fonts = [];
		try {
			update_option( 'qazana_' . self::TYPEKIT_KIT_ID_OPTION_NAME, sanitize_text_field( $_POST['kit_id'] ) );
	...
	protected function actions() {
		parent::actions();
		if ( is_admin() ) {
			add_action( 'qazana/admin/after_create_settings/' . qazana()->slug, [ $this, 'register_admin_fields' ], 100 );
		}
		add_filter( 'qazana/fonts/additional_fonts', [ $this, 'register_fonts_in_control' ] );
		add_action( 'qazana/fonts/print_font_links/' . $this->get_type(), [ $this, 'print_font_link' ] );
		add_action( 'wp_ajax_qazana_admin_fetch_fonts', [ $this, 'integrations_admin_ajax_handler' ] );
	}
}