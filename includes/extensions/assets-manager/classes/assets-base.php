...
abstract class Assets_Base {
	...
	public function print_metabox( $fields ) {
		?>
		<div class="qazana-metabox-content">
			<?php
	...
	public function get_field_label( $field ) {
		...
		return '<p class="qazana-field-label"><label for="' . esc_attr( $id ) . '">' . $field['label'] . '</label></p>';
	}
	...
	public function get_file_field( $field, $saved ) {
		...
		$html .= $this->get_input_field(
			[
				'type' => 'button',
				'class' => 'button qazana-button qazana-upload-btn',
				'name' => $field['id'],
				'id' => $field['id'],
				'value' => '',
				...
				'data-upload_text' => __( 'Upload', 'qazana' ),
				'data-remove_text' => __( 'Delete', 'qazana' ),
	...
	public function get_repeater_field( $field, $saved ) {
		...
		$add_label = isset( $field['add_label'] ) ? $field['add_label'] : __( 'Add item', 'qazana' );
		$row_label = isset( $field['row_label'] ) ? $field['row_label'] : __( 'Row', 'qazana' );
		$row_label_html_args = [
			'id' => 'row_label_' . $js_id,
		...
		echo '<input type="button" class="button qazana-button add-repeater-row" value="' . esc_attr( $add_label ) . '" data-template-id="' . $js_id . '_block">';
	...
	private function get_repeater_tools( $field ) {
		$confirm = isset( $field['confirm'] ) ? $field['confirm'] : __( 'Are you sure?', 'qazana' );
		$remove_title = isset( $field['remove_title'] ) ? $field['remove_title'] : __( 'Delete', 'qazana' );
		$toggle_title = isset( $field['toggle_title'] ) ? $field['toggle_title'] : __( 'Edit', 'qazana' );
		$close_title = isset( $field['close_title'] ) ? $field['close_title'] : __( 'Close', 'qazana' );

		return '<span class="qazana-repeater-tool-btn close-repeater-row" title="' . esc_attr( $close_title ) . '">
                    <i class="eicon-times"></i>' . $close_title . '
                </span>
                <span class="qazana-repeater-tool-btn toggle-repeater-row" title="' . esc_attr( $toggle_title ) . '">
                    <i class="eicon-edit"></i>' . $toggle_title . '
                </span>
                <span class="qazana-repeater-tool-btn remove-repeater-row" data-confirm="' . $confirm . '" title="' . esc_attr( $remove_title ) . '">
	...
	public function get_field_row( $field, $field_html ) {
		...
		$css_id .= ' qazana-field-' . $field['field_type'];

		return '<div class="qazana-field' . $css_id . '">' . $this->get_field_label( $field ) . $field_html . $description . '</div>';
	}
