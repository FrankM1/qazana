<?php
namespace Qazana\Core\Managers;
...
/**
 * Qazana documents manager.
 *
 * Qazana documents manager handler class is responsible for registering and
 * managing Qazana documents.
 *
 * @since 2.0.0
 */
class Documents {
	 ...
	 *
	 * Initializing the Qazana documents manager.
	 *
	 * @since 2.0.0
	 * @access public
	 */
	public function __construct() {
		...
		// add_action( 'qazana_init', [ $this, 'register_default_types' ], 11 );
	...
	public function register_default_types() {
		...
		/**
		 * Register Qazana documents.
		 *
		 * Fires after Qazana registers the default document types.
		 *
		 * @since 2.0.0
		 *
		 * @param Documents $this The document manager instance.
		 */
		do_action( 'qazana/documents/register', $this );
	}
	 ...
	 *
	 * @return Documents The updated document manager instance.
	 */
	...
	public function get( $post_id, $from_cache = true ) {
		...
		$post_id = apply_filters( 'qazana/documents/get/post_id', $post_id );
	...
	public function create( $type, $post_data = [], $meta_data = [] ) {
		...
		if ( empty( $post_data['post_title'] ) ) {
			$post_data['post_title'] = __( 'Qazana', 'qazana' );
			if ( 'post' !== $type ) {
		...
		add_post_meta( $post_id, '_qazana_edit_mode', 'qazana' );
	 ...
	 *
	 * @throws \Exception If current user don't have permissions to edit the post or the post is not using Qazana.
	 *
	...
	public function ajax_save( $request ) {
		...
		if ( ! $document->is_built_with_qazana() || ! $document->is_editable_by_current_user() ) {
		 ...
		 */
		$return_data = apply_filters( 'qazana/documents/ajax_save/return_data', $return_data, $document );
