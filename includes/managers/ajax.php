 ...
 * Qazana ajax manager.
 *
 * Qazana ajax manager handler class is responsible for handling Qazana
...
class Ajax {
	...
	public function register_ajax_action( $tag, $callback ) {
		if ( ! did_action( 'qazana/ajax/register_actions' ) ) {
			_doing_it_wrong( __METHOD__, esc_html( __( 'Use `qazana/ajax/register_actions` hook to register ajax action.', 'qazana' ) ), '2.0.0' );
		}
	 ...
	 *
	 * Fired by `wp_ajax_qazana_ajax` action.
	 *
	 * @since 2.0.0
	 * @access public
	 */
	public function handle_ajax_request() {
		...
		if ( empty( $_REQUEST['actions'] ) || empty( $_REQUEST['editor_post_id'] ) ) {
			$this->add_response_data( false, __( 'Actions and Post ID are required.', 'qazana' ) )
		...
		if ( ! get_post( $editor_post_id ) ) {
			$this->add_response_data( false, __( 'Post not found.', 'qazana' ) )
		 ...
		 */
		do_action( 'qazana/ajax/register_actions', $this );
		...
		foreach ( $this->requests as $id => $action_data ) {
			...
			if ( ! isset( $this->ajax_actions[ $action_data['action'] ] ) ) {
				$this->add_response_data( false, __( 'Action not found.', 'qazana' ), Exceptions::BAD_REQUEST );
	 ...
	 *
	 * Initializing Qazana ajax manager.
	 *
	 * @since 2.0.0
	 * @access public
	 */
	public function __construct() {
		add_action( 'wp_ajax_qazana_ajax', [ $this, 'handle_ajax_request' ] );
	}
}
