...
final class Manager {
	...
	public function register_all_extensions() {
		...
		$this->loader->add_stack(
			...
			qazana()->plugin_extensions_locations
		);
	 ...
	 * Dashes to camelCase
	 *
	 * Used to generate proper class names
	 *
	 * @param string String with dashes
	 * @return string CamelCased String
	 */
	function dashes_to_camelcase( $str ) {
	...
	public function initialize_extension( $folder, $path ) {
		...
		if ( $file = $this->loader->locate_widget( "{$folder}/extension_{$folder}.php", false ) && file_exists( $class_file ) && empty( $this->extensions[ $folder ] ) ) {
			...
			if ( class_exists( $extension_class ) ) {
				...
				$this->load_extension_dependencies( $folder );
			}
		}
