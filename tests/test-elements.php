...
class Qazana_Test_Elements extends WP_UnitTestCase {

	public function test_getInstance() {
		$this->assertInstanceOf( '\Qazana\Elements_Manager', qazana()->elements_manager );
	}

	public function test_getElements() {
		$this->assertNotEmpty( qazana()->elements_manager->get_element_types() );
	}

	public function test_elementMethods() {
		foreach ( qazana()->elements_manager->get_element_types() as $element ) {
	...
	public function test_registerNUnregisterElement() {
		...
		$this->assertTrue( qazana()->elements_manager->register_element_type( new $element_class( [ 'id' => $element_id ] ) ) );

		$element = qazana()->elements_manager->get_element_types( $element_id );
		$this->assertInstanceOf( $element_class, $element );

		$this->assertTrue( qazana()->elements_manager->unregister_element_type( $element_id ) );
		$this->assertFalse( qazana()->elements_manager->unregister_element_type( $element_id ) );

		$this->assertNull( qazana()->elements_manager->get_element_types( $element_id ) );
	...
	public function test_redeclareControl() {
		$element_obj = qazana()->elements_manager->get_element_types( 'section' );
