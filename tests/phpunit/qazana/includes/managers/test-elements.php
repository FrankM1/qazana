...
class Qazana_Test_Elements extends Qazana_Test_Base {
	...
	public function test_getElements() {
		$this->assertNotEmpty( $this->qazana()->elements_manager->get_element_types() );
	}

	public function test_elementMethods() {
		foreach ( $this->qazana()->elements_manager->get_element_types() as $element ) {
	...
	public function test_registerNUnregisterElement() {
		...
		$this->assertTrue( $this->qazana()->elements_manager->register_element_type( new $element_class( [ 'id' => $element_id ] ) ) );

		$element = $this->qazana()->elements_manager->get_element_types( $element_id );
		$this->assertInstanceOf( $element_class, $element );

		$this->assertTrue( $this->qazana()->elements_manager->unregister_element_type( $element_id ) );
		$this->assertFalse( $this->qazana()->elements_manager->unregister_element_type( $element_id ) );

		$this->assertNull( $this->qazana()->elements_manager->get_element_types( $element_id ) );
	...
	public function test_redeclareControl() {
		$element_obj = $this->qazana()->elements_manager->get_element_types( 'section' );
