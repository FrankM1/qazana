...
class Qazana_Test_Revisions_Manager extends Qazana_Test_Base {
	...
	public function test_should_avoid_delete_auto_save() {
		...
		update_post_meta( $post_id, '_qazana_edit_mode', false );
		$res = Revisions_Manager::avoid_delete_auto_save( $post_content, $post_id );
		$this->assertEquals( $post_content, $res, 'post content should not change' );

		update_post_meta( $post_id, '_qazana_edit_mode', true );
		$res = Revisions_Manager::avoid_delete_auto_save( $post_content, $post_id );
		$this->assertEquals( $post_content . '<!-- Created with Qazana -->', $res,
			'post content should change, "<!-- Created with Qazana -->" should be added' );
	}
	...
	public function test_should_remove_temp_post_content() {
		...
		$qazana_addon = '<!-- Created with Qazana -->';
		$post_content = 'post content';
		$post = $this->factory()->create_and_get_custom_post( [ 'post_content' => $post_content . $qazana_addon ] );

		update_post_meta( $post->ID, '_qazana_edit_mode', false );
		Revisions_Manager::remove_temp_post_content();
		$this->assertEquals( $post->post_content, $post_content . $qazana_addon,
			'post content should contain "<!-- Created with Qazana -->"' );

		update_post_meta( $post->ID, '_qazana_edit_mode', true );
		Revisions_Manager::remove_temp_post_content();
		$this->assertEquals( $post->post_content, $post_content,
			'post content should not contain "<!-- Created with Qazana -->"' );
	...
	public function test_should_return_null_from_restore_revision() {
		...
		update_post_meta( $post_id, '_qazana_meta_data', 'content' );
	...
	public function test_should_restore_revision() {
		...
		update_metadata( 'post', $autosave_post_id, '_qazana_edit_mode', true );
		update_metadata( 'post', $autosave_post_id, '_qazana_meta_data', 'content' );
	...
	public function test_should_add_revision_support_for_all_post_types() {
		...
		$supported_types = [ 'page', 'post', 'qazana_library' ];

		foreach ( $supported_types as $supported_type ) {
			$this->assertTrue( post_type_supports( $supported_type, 'qazana' ) );
		}
	}
	...
	private function check_revisions( $post_id, $autosave_post_id ) {
		$parent_qazana_meta_data = get_post_meta( $post_id, '_qazana_meta_data', true );
		$child_qazana_meta_data = get_post_meta( $autosave_post_id, '_qazana_meta_data', true );

		return $child_qazana_meta_data === $parent_qazana_meta_data;
	}
}
